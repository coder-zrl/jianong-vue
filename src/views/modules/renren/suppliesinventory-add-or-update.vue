<template>
  <el-dialog
    ...
    <el-form-item label="物资编号" prop="productId">
      <el-input v-model="dataForm.productId" placeholder="物资编号"></el-input>
    </el-form-item>
    <el-form-item label="物资名称" prop="productName">
      <el-input v-model="dataForm.productName" placeholder="物资名称"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/renren/suppliesinventory/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.productId = data.suppliesInventory.productId
                this.dataForm.productName = data.suppliesInventory.productName
                this.dataForm.inventoryNumber = data.suppliesInventory.inventoryNumber
                this.dataForm.physicalNumber = data.suppliesInventory.physicalNumber
                this.dataForm.inventoryStatus = data.suppliesInventory.inventoryStatus
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/renren/suppliesinventory/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
